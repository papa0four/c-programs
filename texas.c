...
int numArray[] = { 1, 2, -7, 45, 3, -10, -37, 3, -1, 0 };

int main(void)
{
	int * (*sortFun_ptr)(int, int);
	/*REDO can be named anything you want*/
	REDO: printf("Choose a function to test the array of numbers by entering 1 - 4: \n");

	switch (_getch())
	{
		case 49:
			sortFun_ptr = &low_to_high_compare;
			break;
		case 50:
			sortFun_ptr = &high_to_low_compare;
			break;
		case 51:
			sortFun_ptr = &absolute_low_to_high_compare;
			break;
		case 52:
			sortFun_ptr = &absolute_high_to_low_compare;
			break;

		default:
			system("cls");
			goto REDO;//jumps to where your REDO is declared
	}
	
	for (int i = 0; i < 10; i++) { printf("%d ", numArray[i]); }
	printf("\n%d\n", bubble_sort(numArray, 10, sortFun_ptr)); // calling function bubble_sort and return its value at the same time
	for (int i = 0; i < 10; i++) { printf("%d ", numArray[i]); }
	printf("\n");
