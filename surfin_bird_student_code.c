...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
	...
	if (!sentence_ptr) // if the sentence pointer is NULL
	{
		retVal_ptr = NULL; // return NULL
		*errorCode_ptr = ERROR_NULL_SENTENCE_POINTER; // return ERROR code NULL SENTENCE POINTER
	}
	else if (!searchWord_ptr) // if the pointer to the search word is NULL
	{
		retVal_ptr = NULL; // return NULL
		*errorCode_ptr = ERROR_NULL_SEARCH_POINTER; // return ERROR code NULL SEARCH POINTER
	}
	else if (!errorCode_ptr) // if there is no error code pointer
	{
		retVal_ptr = NULL; // return NULL
	}
	else
	{
		senLen = strlen(sentence_ptr); // set temp variable senLen to the length of sentence pointer
		wordLen = strlen(searchWord_ptr); // set temp variable wordLen to the length of search word pointer 
		...
		{
			*errorCode_ptr = ERROR_SEARCH_NOT_FOUND;
			for (i = 0; i < (senLen - wordLen); i++)
			{
				if (*(sentence_ptr + i) == *(searchWord_ptr)) // if your first letter matches the search word
				{
					if (wordLen == 1)
					{
						retVal_ptr = *(sentence_ptr + i);
