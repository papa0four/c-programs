...
int reverse_it(char * forwardString, int strLen)
{
	...
	if (!forwardString) // checks to see if there is a string at all
	{
		return ERR_NULL_POINTER;
	}
	else if (strLen <= 2) // checks string length for 2 or more characters
	{
		return ERR_INVALID_LENGTH;
	}
	else
	{
		for (int i = strLen - 1; i >= 0; --i) // for loop to reverse string i = length of string - 1 for decrement; i >= 0 starts at end of str;
		{
			printf("%c", forwardString[i]); // print each characters of the forwardString array
		}
		return 0;
	}
}
